...
Environment Setup
pythonsc.logging.print_versions()
...
pythonfig, axs = plt.subplots(1, 4, figsize=(15, 4))
...
PanelQuestion it answersTotal counts (full range)Is there a long tail of very high-count spots?Total counts (< 10,000)Where does the low-count cliff actually fall?Genes per spot (full range)Do most spots detect a reasonable number of genes?Genes per spot (< 4,000)What does the bottom of the distribution look like?
...
Filtering
pythonsc.pp.filter_cells(adata, min_counts=5000)
...
pythonsc.pp.normalize_total(adata, inplace=True)
sc.pp.log1p(adata)
...
UMAP Visualization
pythonsc.pl.umap(adata, color=["total_counts", "n_genes_by_counts", "clusters"], wspace=0.4)
...
pythonsc.pl.spatial(adata, img_key="hires", color=["total_counts", "n_genes_by_counts"])
...
pythonsc.tl.rank_genes_groups(adata, "clusters", method="t-test")
...
pythonsc.pl.spatial(adata, img_key="hires", color=["clusters", "CR2"])
...
MERFISH Dataset
pythoncoordinates = pd.read_excel("./pnas.1912459116.sd15.xlsx", index_col=0)
...
This section loads data from Moffitt et al., Science 2018 — a landmark MERFISH study of the mouse hypothalamus. The two supplementary files need to be downloaded manually from the paper.
...
pythonsc.pp.normalize_per_cell(adata_merfish, counts_per_cell_after=1e6)
...
pythonsc.pl.umap(adata_merfish, color="clusters")
